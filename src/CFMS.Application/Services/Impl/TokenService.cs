...
public class TokenService : ITokenService
{
    ...
    public string GenerateRefreshToken(User user)
    {
        ...
        var token = new JwtSecurityToken(
            ...
            _audience,
            claims,
            expires: DateTime.Now.ToLocalTime().AddDays(7),
            signingCredentials: creds
        );
    ...
    {
        return token.RevokedAt != null && DateTime.Now.ToLocalTime() >= token.RevokedAt;
    }

    public async Task RevokeRefreshTokenAsync(RevokedToken token)
    {
        token.RevokedAt = DateTime.Now.ToLocalTime();
    ...
    public bool IsTokenExpired(string token)
    {
        ...
        try
        {
            ...
            return expiryDateTime < DateTime.Now.ToLocalTime();
        }
        catch
        {
            return true;
