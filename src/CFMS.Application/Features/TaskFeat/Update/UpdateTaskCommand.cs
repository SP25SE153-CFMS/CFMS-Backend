...
namespace CFMS.Application.Features.TaskFeat.Update
{
    ...
    {
        public UpdateTaskCommand(string? taskName, Guid? taskTypeId, string? description, int? isHarvest, Guid farmId, DateTime[]? startWorkDate, Guid[]? shiftIds, string? locationType, Guid locationId, IEnumerable<TaskResourceRequest>? taskResources)
        {
            TaskName = taskName;
