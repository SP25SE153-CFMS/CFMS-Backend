...
{
    public class GetWareStockQueryHandler : IRequestHandler<GetWareStockQuery, BaseResponse<object>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetWareStockQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<BaseResponse<object>> Handle(GetWareStockQuery request, CancellationToken cancellationToken)
        {
            ...
            var wareStockFoodResponses = resources
                .SelectMany(r => r.ResourceSuppliers.DefaultIfEmpty(), (resource, supplier) =>
                {
                    ...
                    var resourceSupplier = _unitOfWork.ResourceSupplierRepository.Get(filter: f => f.ResourceId.Equals(resource.ResourceId) && f.IsDeleted == false).FirstOrDefault();
