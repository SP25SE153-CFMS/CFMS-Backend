...
namespace CFMS.Application.Features.CategoryFeat.GetSub
{
    public class GetSubQueryHandler : IRequestHandler<GetSubQuery, BaseResponse<SubCategory>>
    {
        ...
        {
            var existSubCategory = _unitOfWork.SubCategoryRepository.Get(filter: c => c.SubCategoryId.Equals(request.SubCategoryId) && c.IsDeleted == false, includeProperties: "SubCategories").FirstOrDefault();
